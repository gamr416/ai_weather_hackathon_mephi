"""Evaluate checkpoint: metrics JSON + original vs reconstruction plots."""
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import torch
import yaml
from tqdm import tqdm

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from src.data.era5 import CHANNEL_ORDER, Era5ZarrDataset, load_stats
from src.models.perceiver_ae import PerceiverAE

SURFACE = CHANNEL_ORDER[:8]
PRESSURE = CHANNEL_ORDER[8:]
PLOT_CHANNELS = ["t2m", "mslp", "tp6h", "tcwv", "T850", "Z850", "U850", "Q850"]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--ckpt", type=Path, required=True, help="best.pt / last.pt")
    p.add_argument("--data", type=Path, required=True, help="…/era5_….zarr root with train/val/test")
    p.add_argument("--split", choices=["val", "test", "train"], default="val")
    p.add_argument("--stats", type=Path, default=None, help="norm_stats.npz (default: next to ckpt)")
    p.add_argument("--out", type=Path, default=None, help="output dir (default: <ckpt_dir>/eval_<split>)")
    p.add_argument("--max-frames", type=int, default=32, help="cap frames for metrics (RAM/time)")
    p.add_argument("--plot-frames", type=int, default=3, help="how many frames to visualize")
    p.add_argument("--device", type=str, default=None)
    p.add_argument("--full-frame", action="store_true", help="eval on full 360x720 (heavier)")
    return p.parse_args()


def build_model(cfg: dict, state: dict, device: torch.device) -> PerceiverAE:
    mcfg = cfg["model"]
    model = PerceiverAE(
        in_channels=28,
        patch_size=mcfg["patch_size"],
        dim=mcfg["dim"],
        num_latents=mcfg["num_latents"],
        depth=mcfg["depth"],
        heads=mcfg["heads"],
        codebook_size=mcfg["codebook_size"],
        dropout=mcfg.get("dropout", 0.0),
    )
    model.load_state_dict(state)
    model.to(device).eval()
    return model


@torch.no_grad()
def collect_errors(
    model: PerceiverAE,
    ds: Era5ZarrDataset,
    device: torch.device,
    max_frames: int,
) -> tuple[np.ndarray, float, list[dict]]:
    """Return per-channel weighted SSE, weight sum, and example frames."""
    n = min(len(ds), max_frames)
    sse = np.zeros(28, dtype=np.float64)
    wsum = 0.0
    examples: list[dict] = []

    for i in tqdm(range(n), desc="eval"):
        batch = ds[i]
        x = batch["x"].unsqueeze(0).to(device)
        w = batch["lat_weight"].to(device)  # H
        out = model(x)
        recon = out["recon"][0].cpu()
        target = batch["x"]
        # denormalize for plots / physical-ish look
        mean = ds.mean.cpu()
        std = ds.std.cpu()
        recon_phys = recon * std + mean
        target_phys = target * std + mean

        ww = w.cpu().numpy().astype(np.float64)  # H
        # broadcast to H,W
        w2 = ww[:, None]
        err = (recon.numpy() - target.numpy()) ** 2  # C,H,W
        sse += (err * w2[None, :, :]).sum(axis=(1, 2))
        wsum += float(w2.sum() * target.shape[-1])

        if len(examples) < 8:
            examples.append(
                {
                    "index": int(batch["index"]),
                    "target": target_phys.numpy(),
                    "recon": recon_phys.numpy(),
                    "lat": ds.ds["lat"].values if ds.crop_size is None else None,
                }
            )
    return sse, wsum, examples


def scores_from_sse(sse: np.ndarray, wsum: float, std: np.ndarray) -> dict:
    rmse = np.sqrt(sse / max(wsum, 1e-12))
    nrmse = rmse / np.maximum(std.reshape(28), 1e-6)
    surface = float(nrmse[:8].mean())
    pressure = float(nrmse[8:].mean())
    per = {CHANNEL_ORDER[i]: {"rmse_norm": float(rmse[i]), "nrmse": float(nrmse[i])} for i in range(28)}
    return {
        "S_surface": surface,
        "S_pressure": pressure,
        "S_all": 0.5 * surface + 0.5 * pressure,
        "per_channel": per,
        "note": "NRMSE in normalized space units used at train time; compare runs relative to each other. VAEformer CI comparison needs official evaluator.",
    }


def plot_frame_comparison(
    target: np.ndarray,
    recon: np.ndarray,
    out_path: Path,
    frame_id: int,
    channels: list[str],
) -> None:
    cols = len(channels)
    fig, axes = plt.subplots(3, cols, figsize=(3.2 * cols, 8), constrained_layout=True)
    if cols == 1:
        axes = np.asarray(axes).reshape(3, 1)
    for j, name in enumerate(channels):
        ci = CHANNEL_ORDER.index(name)
        t = target[ci]
        r = recon[ci]
        d = r - t
        vmax = np.nanpercentile(np.abs(t), 99)
        vmin = -vmax if name.startswith(("U", "V", "u", "v")) else np.nanpercentile(t, 1)
        vmax_t = np.nanpercentile(t, 99) if not name.startswith(("U", "V", "u", "v")) else vmax
        for row, img, title, cmap, v0, v1 in [
            (0, t, f"{name} truth", "viridis", vmin if name.startswith(("U", "V")) else np.nanpercentile(t, 1), vmax_t),
            (1, r, f"{name} recon", "viridis", vmin if name.startswith(("U", "V")) else np.nanpercentile(t, 1), vmax_t),
            (2, d, f"{name} diff", "RdBu_r", -np.nanpercentile(np.abs(d), 99), np.nanpercentile(np.abs(d), 99)),
        ]:
            ax = axes[row, j]
            im = ax.imshow(img, origin="lower", cmap=cmap, vmin=v0, vmax=v1, aspect="auto")
            ax.set_title(title, fontsize=9)
            ax.set_xticks([])
            ax.set_yticks([])
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    fig.suptitle(f"Frame {frame_id}: truth vs reconstruction", fontsize=12)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, dpi=140)
    plt.close(fig)


def plot_nrmse_bars(metrics: dict, out_path: Path) -> None:
    names = CHANNEL_ORDER
    vals = [metrics["per_channel"][n]["nrmse"] for n in names]
    colors = ["#3b82f6"] * 8 + ["#10b981"] * 20
    fig, ax = plt.subplots(figsize=(12, 4), constrained_layout=True)
    ax.bar(range(28), vals, color=colors)
    ax.set_xticks(range(28))
    ax.set_xticklabels(names, rotation=90, fontsize=7)
    ax.set_ylabel("NRMSE")
    ax.set_title(
        f"Per-channel NRMSE  |  S_all={metrics['S_all']:.3f}  "
        f"S_surface={metrics['S_surface']:.3f}  S_pressure={metrics['S_pressure']:.3f}"
    )
    ax.axvline(7.5, color="gray", ls="--", lw=0.8)
    fig.savefig(out_path, dpi=140)
    plt.close(fig)


def plot_history(history_path: Path, out_path: Path) -> None:
    if not history_path.exists():
        return
    hist = json.loads(history_path.read_text())
    steps, losses, recs = [], [], []
    vsteps, vlosses = [], []
    for row in hist:
        if "loss" in row:
            steps.append(row["step"])
            losses.append(row["loss"])
            recs.append(row.get("recon"))
        if "val_loss" in row:
            vsteps.append(row["step"])
            vlosses.append(row["val_loss"])
    fig, ax = plt.subplots(figsize=(7, 4), constrained_layout=True)
    if steps:
        ax.plot(steps, losses, label="train loss", lw=1.5)
        if any(r is not None for r in recs):
            ax.plot(steps, recs, label="train recon", lw=1.2, alpha=0.8)
    if vsteps:
        ax.plot(vsteps, vlosses, label="val loss", marker="o", lw=1.5)
    ax.set_xlabel("step")
    ax.set_ylabel("loss")
    ax.set_title("Training curves")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.savefig(out_path, dpi=140)
    plt.close(fig)


def main() -> None:
    args = parse_args()
    ckpt = torch.load(args.ckpt, map_location="cpu", weights_only=False)
    cfg = ckpt.get("cfg") or yaml.safe_load((ROOT / "configs/perceiver_0p5.yaml").read_text())
    device = torch.device(args.device or ("cuda" if torch.cuda.is_available() else "cpu"))

    stats_path = args.stats or (args.ckpt.parent / "norm_stats.npz")
    if not stats_path.exists():
        raise SystemExit(f"Missing stats: {stats_path}")
    mean, std = load_stats(stats_path)

    split_path = args.data / f"{args.split}.zarr"
    if not split_path.exists():
        raise SystemExit(f"Missing split: {split_path}")

    crop = None if args.full_frame else cfg["data"].get("crop_size", 192)
    ds = Era5ZarrDataset(
        split_path,
        mean=mean,
        std=std,
        crop_size=crop,
        augment=False,
        seed=0,
    )
    model = build_model(cfg, ckpt["model"], device)

    out_dir = args.out or (args.ckpt.parent / f"eval_{args.split}")
    out_dir.mkdir(parents=True, exist_ok=True)

    sse, wsum, examples = collect_errors(model, ds, device, args.max_frames)
    metrics = scores_from_sse(sse, wsum, std)
    metrics["split"] = args.split
    metrics["n_frames"] = min(len(ds), args.max_frames)
    metrics["ckpt"] = str(args.ckpt)
    metrics["crop_size"] = crop
    (out_dir / "metrics.json").write_text(json.dumps(metrics, indent=2))

    plot_nrmse_bars(metrics, out_dir / "nrmse_bars.png")
    plot_history(args.ckpt.parent / "history.json", out_dir / "train_curves.png")

    for k, ex in enumerate(examples[: args.plot_frames]):
        plot_frame_comparison(
            ex["target"],
            ex["recon"],
            out_dir / f"compare_frame{ex['index']:04d}.png",
            ex["index"],
            PLOT_CHANNELS,
        )

    print(json.dumps({k: metrics[k] for k in ("S_all", "S_surface", "S_pressure", "n_frames")}, indent=2))
    print(f"Wrote {out_dir}/metrics.json and plots")


if __name__ == "__main__":
    main()
