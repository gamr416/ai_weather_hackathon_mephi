"""Evaluate Residual-FSQ checkpoint: metrics + plots + raw CR breakdown."""
...
from src.models.residual_fsq import ResidualFSQAE, raw_cr_from_bits

PLOT_CHANNELS = ["t2m", "mslp", "tp6h", "tcc", "T850", "Z850", "U850", "Q850"]
SST_IDX = 5
...
def build_model(cfg: dict, state: dict, device: torch.device) -> ResidualFSQAE:
    mcfg = cfg["model"]
    model = ResidualFSQAE(
        in_channels=28,
...
def collect_errors(
    ...
    wsum = np.zeros(28, dtype=np.float64)
    examples: list[dict] = []
    ...
    for i in tqdm(range(n), desc="eval"):
        ...
        rate_c_total += float(out["rate_bits_c"].cpu())
        rate_f_total += float(out["rate_bits_f"].cpu())
        ...
        # ocean mask for SST (static ch0 = land_sea_mask)
        lsm = batch["static"][0].numpy()
        ocean = (1.0 - lsm).astype(np.float64)
        spatial_w = np.broadcast_to(w2, (h, w_img)).copy()
        ch_w = np.stack([spatial_w] * 28, axis=0)
        ch_w[SST_IDX] = spatial_w * ocean

        sse_norm += (err_n * ch_w).sum(axis=(1, 2))
        sse_phys += (err_p * ch_w).sum(axis=(1, 2))
        wsum += ch_w.sum(axis=(1, 2))
    ...
    codec = {
        ...
        "mean_rate_bits_coarse": rate_c_total / max(n, 1),
        "mean_rate_bits_fine": rate_f_total / max(n, 1),
        "expected_raw_cr": float(model.expected_raw_cr()),
        "note": "raw CR from coarse+fine uniform indices (no entropy coding yet)",
    }
    return sse_norm, sse_phys, wsum, examples, codec


def scores_from_sse(sse_norm: np.ndarray, sse_phys: np.ndarray, wsum: np.ndarray) -> dict:
    rmse_norm = np.sqrt(sse_norm / np.maximum(wsum, 1e-12))
    nrmse = rmse_norm.copy()
    rmse_phys = np.sqrt(sse_phys / np.maximum(wsum, 1e-12))
...
def plot_history(history_path: Path, out_path: Path) -> None:
    ...
    ax.set_xlabel("step")
    ax.set_ylabel("loss")
    ax.set_title("Residual-FSQ training curves")
    ax.legend()
...
def main() -> None:
    ...
    cfg = ckpt.get("cfg") or yaml.safe_load((ROOT / "configs/residual_fsq_0p5.yaml").read_text())
        ...
        json.dumps(
            {
                ...
                "rate_bits_coarse": codec["mean_rate_bits_coarse"],
                "rate_bits_fine": codec["mean_rate_bits_fine"],
                "n_frames": metrics["n_frames"],
                "nparams_m": model.num_parameters() / 1e6,
            },
            indent=2,
        )
    )
